...
from quartzbio.utils.files import check_gzip_path
...
from quartzbio.client import QuartzBioClient
from quartzbio.client import client as global_client
...
def _check_uploaded_folders(
    base_remote_path, local_start, all_folders, follow_shortcuts=False
):
        ...
        if follow_shortcuts:
            ...
            try:
                Object.get_by_full_path(
                    full_path=folder_full_path, follow_shortcuts=True
                )
    ...
    for vault, remote_folder_path in all_folders:
        ...
        for folder in subfolders[1:]:
            ...
            if not _folder_exists(
                folder_full_path, remote_folders_existing, follow_shortcuts
            ):
...
def _upload_folder(
    ...
    try:
        ...
        Object.get_by_full_path(
            upload_root_path, assert_type="folder", follow_shortcuts=follow_shortcuts
        )
    except NotFoundError:
    ...
    for abs_local_parent_path, folders, files in os.walk(base_local_path):
        ...
        client_auth = (
            global_client._host,
            global_client._auth.token,
            global_client._auth.token_type,
        )
        for f in files:
    ...
    if num_processes > 1:
        ...
        all_folder_parts = _check_uploaded_folders(
            base_remote_path, local_start, all_folders, follow_shortcuts
        )
    else:
...
def import_file(args):
    ...
    if args.create_dataset:
        ...
        except quartzbio.errors.NotFoundError:
    ...
    if args.follow:
        ...
        mesh_url = "https://my.quartzbio.com/activity/"
...
def _download_recursive(
    ...
    results = list(
        _resolve_shortcuts_and_get_files(
            full_path=full_path, follow_shortcuts=follow_shortcuts
        )
    )
    ...
    if num_files == 0:
        ...
        if follow_shortcuts:
            num_shortcuts = len(
                [x for x in results if x.get("object_type") == "shortcut"]
            )
            (
                print(
                    "{} shortcuts found on path. Use --follow-shortcuts flag to download.".format(
                        num_shortcuts
                    )
                )
                if num_shortcuts > 0
    ...
    for remote_file in remote_files:
        ...
        if not dry_run:
            ...
            if num_processes is not None:
                file = {"path": local_path, "file": remote_file}
...
def _download_in_parallel(files_to_download, num_processes):
    def _download_worker(file_info):
        try:
            ...
            print(
                "Error occurred while downloading file: ({}).".format(
                    file_info.get("path")
                )
            )
            raise e
    ...
    if num_processes <= 0:
        ...
        print(
            "[Warning] num-processes cannot be less than 1. Defaulting to CPU count: ({})".format(
                num_processes
            )
        )
...
def _resolve_shortcuts_and_get_files(
    full_path, download_path=None, follow_shortcuts=False
):
    ...
    files = []
    for x in results:
        x.path = download_path + _get_relative_download_path(
            base_path=full_path, path_to_object=x.full_path, filename=x.name
        )
        files.append(x)
    ...
    for shortcut in shortcuts:
        ...
        try:
            ...
            print(
                ...
                "has been moved, deleted or you don't have permissions to view it.".format(
                    shortcut.full_path
                )
            )
            continue
        ...
        if not target_object:
            print(
                "Couldn't find target object for shortcut: {}".format(
                    shortcut.full_path
                )
            )
            continue
            ...
            print(
                "Found URL shortcut at: ({}) skipping download.".format(
                    shortcut_object.full_path
                )
            )
            continue
...
def ls(args):
    ...
    if "**" in args.full_path:
        print(
            "Recursive paths containing '**' are not supported by `ls`. "
            "Try the --recursive flag instead."
        )
        return False

    files = _ls(
        args.full_path, recursive=args.recursive, follow_shortcuts=args.follow_shortcuts
    )
        ...
        if follow_shortcuts and file_.is_shortcut:
            ...
            try:
                ...
                print(
                    ...
                    "the target may have been deleted or you may not have permission to access it".format(
                        shortcut
                    )
                )
        else:
