...
from .quartzbio_object import QuartzBioObject, convert_to_quartzbio_object


class APIResource(QuartzBioObject):
...
class ListObject(QuartzBioObject):
    def all(self, **params):
    ...
    def quartzbio_objects(self):
        return convert_to_quartzbio_object(self["data"], client=self._client)

    def set_tabulate(self, fields, **kwargs):
        self._tabulate = lambda data: tabulate(
            [[d[i] for i in fields] for d in data], **kwargs
        )
    ...
    def next(self):
        ...
        obj = convert_to_quartzbio_object(self["data"][self._i], client=self._client)
        self._i += 1
        return obj
...
class SingletonAPIResource(APIResource):
    ...
    def class_url(cls):
        ...
        base = "v{0}".format(getattr(cls, "RESOURCE_VERSION", "1"))
        return "/{0}/{1}".format(
            base, class_to_api_name(cls.class_name(), pluralize=False)
        )
...
class CreateableAPIResource(APIResource):
    ...
    def create(cls, **params):
        ...
        return convert_to_quartzbio_object(response, client=_client)


class DeletableAPIResource(APIResource):
    def delete(self, **params):
        printable_name = class_to_api_name(self.class_name(), pluralize=False).replace(
            "_", " "
        )
...
class ListableAPIResource(APIResource):
    ...
    def _retrieve_helper(cls, model_name, field_name, error_value, **params):
        ...
        if len(objects) > 1:
            if allow_multiple:
                ...
                raise Exception(
                    'Multiple {0}s found with {1} "{2}"'.format(
                        model_name, field_name, error_value
                    )
                )
        elif len(objects) == 1:
            return objects[0]
    ...
    def __repr__(self):
        return tabulate(
            list(self.items()), ["Fields", "Data"], aligns=["right", "left"], sort=True
        )
...
class UpdateableAPIResource(APIResource):
    def save(self):
        self.refresh_from(
            self.request("patch", self.instance_url(), data=self.serialize(self))
        )
        return self
    ...
    def serialize(self, obj):
        ...
        for k in unsaved_values:
            ...
            if isinstance(attr, QuartzBioObject):
                params[k] = dict(attr)
            else:
                params[k] = attr
