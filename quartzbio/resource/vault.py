...
class Vault(
    ...
    def _get_parent_folder(self, path):
        ...
        return Object.get_by_full_path(
            ":".join([self.full_path, path]), assert_type="folder", client=self._client
        )
    ...
    def create_folder(self, filename, **params):
        ...
        params.update(
            {"filename": filename, "vault_id": self.id, "object_type": "folder"}
        )
    ...
    def get_or_create_uploads_path(cls, **kwargs):
        ...
        try:
            ...
            print("Uploads directory not found. Creating {0}".format(full_path))
