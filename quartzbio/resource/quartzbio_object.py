...
def convert_to_quartzbio_object(resp, **kwargs):
    from . import types

    _client = kwargs.pop("client", None)

    if isinstance(resp, list):
        return [convert_to_quartzbio_object(i, client=_client) for i in resp]
...
class QuartzBioObject(dict):
    """Base class for all QuartzBio API resource objects"""
    ...
    def __init__(self, id=None, **params):
        super(QuartzBioObject, self).__init__()
    ...
    def __setattr__(self, k, v):
        if k[0] == "_" or k in self.__dict__:
            return super(QuartzBioObject, self).__setattr__(k, v)
        else:
            self[k] = v
    ...
    def __setitem__(self, k, v):
        super(QuartzBioObject, self).__setitem__(k, v)
