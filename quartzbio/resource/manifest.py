...
class Manifest(object):
    ...
    def add_file(self, path, **kwargs):
        default_upload_path = quartzbio.Vault.get_or_create_uploads_path()
        vault = quartzbio.Vault.get_personal_vault()
        print("Uploading file: {0} to {1}".format(path, default_upload_path))
        file_ = quartzbio.Object.upload_file(path, default_upload_path, vault.full_path)
        print(
            "Successfuly uploaded file {0} (id:{1} size:{2} md5:{3})".format(
                path, file_.id, file_.size, file_.md5
            )
        )
    ...
    def add(self, *args):
        ...
        All files are uploaded to QuartzBio. The Upload
        ...
        for path in args:
            ...
            if _is_supported_url(path):
                ...
                raise ValueError(
                    ...
                    "or URLs with http:// or https://.".format(path)
                )
