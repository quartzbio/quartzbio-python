...
from quartzbio.utils.files import separate_filename_extension
...
class Object(
    ...
    though more may be added later: folder, file, and QuartzBio Dataset.
    """
    ...
    def validate_full_path(cls, full_path, **kwargs):
        ...
        """
        from quartzbio.resource.vault import Vault
        ...
        try:
            vault_full_path, path_dict = Vault.validate_full_path(
                input_vault, client=_client
            )
        except Exception as err:
            raise Exception(
                'Could not determine vault from "{0}": {1}'.format(input_vault, err)
            )
        ...
        if match:
            ...
            raise Exception(
                ...
                "Full path must be in one of the following formats: "
                '"vault:/path", "domain:vault:/path", or "~/path"'.format(full_path)
            )
        ...
        path_dict["parent_full_path"] = "{vault_full_path}:{parent_path}".format(
            **path_dict
        )
    ...
    def get_by_path(cls, path, **params):
        ...
        if obj and assert_type and obj["object_type"] != assert_type:
            raise QuartzBioError(
                "Expected a {} but found a {} at {}".format(
                    assert_type, obj["object_type"], path
                )
            )
    ...
    def get_by_full_path(cls, full_path, **params):
        ...
        obj = cls._retrieve_helper("object", "full_path", full_path, **params)
        if obj and assert_type and obj["object_type"] != assert_type:
            raise QuartzBioError(
                "Expected a {} but found a {} at {}".format(
                    assert_type, obj["object_type"], full_path
                )
            )
    ...
    def get_or_create_by_full_path(cls, full_path, **kwargs):
        ...
        assert_type = kwargs.pop("assert_type", kwargs.get("object_type", None))
        ...
        if not object_type:
            raise Exception(
                "'object_type' is required when creating a new "
        ...
        if create_vault:
            vault = Vault.get_or_create_by_full_path(
                "{0}:{1}".format(parts["domain"], parts["vault"]), client=_client
            )
        else:
            vaults = Vault.all(
                account_domain=parts["domain"], name=parts["vault"], client=_client
            )
            if len(vaults.quartzbio_objects()) == 0:
                raise Exception(
            ...
            vault = vaults.quartzbio_objects()[0]
        ...
        while curr_path != "/":
            try:
                obj = Object.get_by_path(
                    curr_path, vault_id=vault.id, assert_type="folder", client=_client
                )
    ...
    def _archive(self, archive_folder):
        if not self.object_type == "file":
            raise NotImplementedError(
                "Object archiving is only supported for files, not {}".format(
                    self.object_type
                )
            )
        ...
        base_filename, file_extension, compression = separate_filename_extension(
            self.filename
        )
        ...
        archive_path = os.path.join(
            archive_folder, os.path.dirname(self.path).lstrip("/"), archive_filename
        )
    ...
    def create_folder(cls, vault, full_path, tags=None, **kwargs):
        ...
        try:
            ...
            print(
                "Notice: Folder created for {0} at {1}".format(
                    folder_name, new_obj.path
                )
            )
    ...
    def create_shortcut(self, shortcut_full_path, **kwargs):
        ...
        full_path, path_dict = Object.validate_full_path(
            shortcut_full_path, client=_client
        )
        ...
        try:
            ...
            target = {"id": self.id, "object_type": self.object_type}

            shortcut = Object.get_or_create_by_full_path(
                full_path=shortcut_full_path,
    ...
    def upload_file(cls, local_path, remote_path, vault_full_path, **kwargs):
        ...
        Args:
            ...
            **kwargs: Additional options:
                ...
                num_processes (int): Number of parallel workers for multipart upload (default: 1)
                max_retries (int): Maximum retries per part for multipart upload (default: 3)
            ...
            if not obj.is_file:
                ...
                if obj.md5 == local_md5:
                    ...
                    if kwargs.get("archive_folder"):
                        ...
                        print(
                            "WARNING: File {} exists on QuartzBio with different "
    ...
    def _upload_single_file(cls, obj, local_path, **kwargs):
        ...
        while True:
            try:
                ...
                n_retries += 1
                print(
                    "WARNING: Retrying ({}/{}) failed upload for {}: {}".format(
        ...
        if upload_resp.status_code != 200:
            ...
            print(
                "Notice: Successfully uploaded {0} to {1}".format(local_path, obj.path)
            )
    ...
    def _object_list_helper(self, **params):
        ...
        if not self.is_folder:
            raise QuartzBioError(
                "Only folders contain child objects. This is a {}".format(
                    self.object_type
                )
            )
    ...
    def __getattr__(self, name):
        ...
        from quartzbio.resource import Dataset
        ...
        try:
            ...
            if name in valid_dataset_attrs and self["object_type"] == "dataset":
                return getattr(Dataset(self["id"], client=self._client), name)
    ...
    def dataset(self):
        ...
        if not self.is_dataset:
            raise QuartzBioError(
                "Only dataset objects have a Dataset resource. This is a {}".format(
                    self.object_type
                )
            )
    ...
    def get_target(self, return_none_target=True):
        if not self.is_shortcut:
            raise QuartzBioError(
                "Only shortcut objects have a target resource. This is a {}".format(
                    self.object_type
                )
            )
        target = self["target"]
        if not target:
            if return_none_target:
                ...
                raise QuartzBioError("Shortcut target not found.")
    ...
    @property
    def data_url(self):
        return "/v2/objects/{}/data".format(self.quartzbio_id)
    ...
    def tag(self, tags, remove=False, dry_run=False, apply_save=True):
        ...
        if remove:
            ...
            if removal_tags:
                print(
                    "{}Notice: Removing tags: {} from object: {}".format(
                ...
                print(
                    "{}Notice: Object {} does not contain any of the "
                    "following tags: {}".format(
                        "[Dry Run] " if dry_run else "", self.full_path, ", ".join(tags)
                    )
                )
                return False
        else:
            ...
            if new_tags:
                print(
                    "{}Notice: Adding tags: {} to object: {}".format(
                        "[Dry Run] " if dry_run else "",
                        ", ".join(new_tags),
                        self.full_path,
                    )
                ...
                print(
                    "{}Notice: Object {} already contains these tags: {}".format(
                        "[Dry Run] " if dry_run else "", self.full_path, ", ".join(tags)
                    )
                )
                return False
    ...
    def query(self, **params):
        ...
        """
        from quartzbio.resource.dataset import Dataset
        from quartzbio.query import QueryFile
        ...
        if self.is_dataset:
            ...
            raise QuartzBioError(
                "The functionality is only supported for files and datasets. "
    ...
    def archive(self, storage_class=None, follow=False):
        ...
        """
        if not self.is_dataset:
            raise QuartzBioError("Only dataset objects can be archived.")
    ...
    def restore(self, storage_class=None, follow=False, **kwargs):
        ...
        """
        if not self.is_dataset:
            raise QuartzBioError("Only datasets can be restored.")
    ...
    def enable_global_beacon(self):
        ...
        """
        if not self.is_dataset:
            raise QuartzBioError("Only dataset objects can be Global Beacons.")
    ...
    def list_versions(self, include_deleted=True):
        ...
        """
        if not self.is_file:
            raise QuartzBioError("Only file objects can have versions.")

        _client = self._client
        url = self.instance_url() + "/versions"
        params = {"show_deleted": include_deleted}
        response = _client.get(url, params)
        results = convert_to_quartzbio_object(response, client=_client)
        # set up tabulate:
