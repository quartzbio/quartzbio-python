...
class QuartzBioAuth(OAuthBase):
    ...
    TOKEN_COOKIE_NAME = "quartzbio_oauth_token"
    ...
    def __init__(self, app, app_url, client_id, **kwargs):
        ...
        self._oauth_redirect_uri = urljoin(
            self._app_url,
            "{}_oauth-redirect".format(self._app.config["requests_pathname_prefix"]),
        )

        # Handle optional parameters
        self._quartzbio_url = kwargs.get("quartzbio_url")
        if not self._quartzbio_url:
            raise Exception(
        ...
        if self._oauth_grant_type == "implicit":
            ...
            raise Exception(
                'QuartzBioAuth Error: Unsupported grant type "{}"'.format(
            ...
            json.dumps(
                {
                    ...
                    "quartzbio_url": self._quartzbio_url,
                    "requests_pathname_prefix": self._app.config[
    ...
    def check_view_access(self, oauth_token):
        client = quartzbio.QuartzBioClient(token=oauth_token, token_type="Bearer")
        try:
    ...
    def login_api(self):
        ...
        if self._oauth_grant_type == "authorization_code":
            ...
            raise Exception(
                'QuartzBio Auth Error: unsupported grant type "{}"'.format(
                    self._oauth_grant_type
                )
            )
        ...
        if not oauth_token:
            ...
            return flask.Response(
                json.dumps(oauth_data), mimetype="application/json", status=500
            )

        client = quartzbio.QuartzBioClient(token=oauth_token, token_type="Bearer")
        user = client.User.retrieve()
        response = flask.Response(
            json.dumps(user), mimetype="application/json", status=200
        )
