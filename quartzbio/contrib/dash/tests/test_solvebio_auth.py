...
from quartzbio.contrib.dash import QuartzBioAuth
...
class ProtectedViewsTest(unittest.TestCase):
    ...
    def test_protecting_all_views(self):
        ...
        self.assertEqual(
            ...
            len([k for k in apps["unregistered"].server.url_map.iter_rules()]) - 1,
        )
    ...
    def test_unauthenticated_view(self):
        ...
        for app_name in ["unregistered"]:
            ...
            for endpoint in (
                endpoints["protected"]["get"] + endpoints["unprotected"]["get"]
            ):
                res = client.get(endpoint)
                test_name = "{} at {} ({})".format(res.status_code, endpoint, app_name)
    ...
    def check_endpoints(self, auth, app, oauth_token, cookies=tuple(), all_200=False):
        def get_client():
            client = app.server.test_client()
            client.set_cookie("/", "quartzbio_oauth_token", oauth_token)
            for cookie in cookies:
                client.set_cookie("/", cookie["name"], cookie["value"])
            return client

        for endpoint in endpoints["unprotected"]["get"] + endpoints["protected"]["get"]:
            client = get_client()  # use a fresh client for every endpoint
            res = client.get(endpoint)
            test_name = "{} at {} as {}".format(res.status_code, endpoint, oauth_token)
            self.assertEqual(res.status_code, 200, test_name)
        return res
    ...
    def test_protected_endpoints_with_auth_cookie(self):
        ...
        for app_name, app in iteritems(apps):
            if app_name != "unregistered":
                app.layout = html.Div()
                self.check_endpoints(auths[app_name], app, self._oauth_token)

    def test_auth_cookie_caches_calls_to_quartzbio(self):
        app = dash.Dash()
        app.scripts.config.serve_locally = True
        auth = QuartzBioAuth(
            app,
        ...
        f = "quartzbio.contrib.dash.quartzbio_auth.QuartzBioAuth.check_view_access"  # noqa
        with mock.patch(f, wraps=auth.check_view_access) as wrapped:
            ...
            n_endpoints = len(endpoints["protected"]["get"]) + len(
                endpoints["unprotected"]["get"]
            )

            self.assertEqual(wrapped.call_count, n_endpoints * 2)

            access_granted_cookie = get_cookie(res, auth.AUTH_COOKIE_NAME)
            ...
            # QuartzBio should be made
            time.sleep(10)
            self.check_endpoints(auth, app, creator)
            self.assertEqual(wrapped.call_count, (n_endpoints * 4))
...
class LoginFlow(unittest.TestCase):
    def login_success(self):
        ...
        QuartzBioAuth(
            app, "https://quartzbio-dash-auth-app.herokuapp.com", oauth_client_id
        )
        app.layout = html.Div()
        client = app.server.test_client()
        ...
        self.assertEqual(get_cookie(res, "quartzbio_oauth_token"), oauth_token)
