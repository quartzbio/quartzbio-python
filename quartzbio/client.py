...
def _handle_api_error(response):
    ...
    raise QuartzBioError(response=response)


def _handle_request_error(e):
    if isinstance(e, requests.exceptions.RequestException):
        msg = QuartzBioError.default_message
        err = "%s: %s" % (type(e).__name__, str(e))
    else:
        msg = (
            "Unexpected error communicating with QuartzBio.\n"
...
class QuartzBioClient(object):
    ...
    def __init__(
        ...
        self._host: str = None
        self._auth: QuartzBioTokenAuth = None
        ...
        if self.retry_all is None:
            self.retry_all = bool(os.environ.get("QUARTZBIO_RETRY_ALL"))
        ...
        if include_resources:
            ...
            for name, class_ in inspect.getmembers(quartzbio, inspect.isclass):
                if name in skip:
                    continue
    ...
    def request(self, method, url, **kwargs):
        ...
        if not self.is_logged_in():
            raise QuartzBioError("HTTP request: client is not logged in!")
        ...
        try:
            ...
            if b"<!DOCTYPE html>" in response.content[:40]:
                ...
                raise QuartzBioError("No EDP API host was set!")
    ...
    def _log_raw_request(self, method, url, **kwargs):
        ...
        req = Request(
            method=method.upper(), url=url, data=kwargs["data"], params=kwargs["params"]
        )
    ...
    def __repr__(self):
        return "<QuartzBioClient {0} {1}>".format(self._host, self._auth)


client = QuartzBioClient(include_resources=False)
