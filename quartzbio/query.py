    ...
    def __init__(self, *raw_filters, **filters):
        ...
        for flt in raw_filters:
            try:
                ...
                raise Exception(
                    "Invalid raw filter, must be a JSON string: {}".format(flt)
                )
...
class GenomicFilter(Filter):
    ...
    # Standardized fields for genomic coordinates in QuartzBio
    ...
    def __init__(self, chromosome, start, stop=None, exact=False):
        ...
        try:
            ...
            raise ValueError("Start and stop positions must be integers (or None)")
        ...
        if exact or start is None:
            ...
            f = Filter(
                **{self.FIELD_START + "__lte": start, self.FIELD_STOP + "__gte": stop}
            )
            if start != stop:
                f = f | Filter(**{self.FIELD_START + "__range": [start, stop]})
                f = f | Filter(**{self.FIELD_STOP + "__range": [start, stop]})
...
class QueryBase(object):
    ...
    # Special case for Query/QueryFile class to pre-set QuartzBioClient
    _client = None
    ...
    def __len__(self):
        ...
        """
        return (
            min(self._limit, self.count()) if not self.count() is None else self._limit
        )
    ...
    def __repr__(self):
        ...
        if getattr(self, "_is_join", False):
            ...
            return "\n%s\n\n... %s." % (
                tabulate(
                    list(enumerate(self._buffer[:10])),
                    ["Row", "Data"],
                    aligns=["right", "left"],
                    is_tsv=is_tsv,
                ),
...
class Query(QueryBase):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not 0 < self._page_size <= self.MAX_PAGE_SIZE:
            raise Exception(
                "'page_size' parameter must be in " "range [1, {}]".format(
                    self.MAX_PAGE_SIZE
                )
            )

        # Set up the QuartzBioClient
    ...
    def execute(self, offset=0, **query):
        ...
        _params.update(
            offset=self._page_offset, limit=min(self._page_size, self._limit)
        )
        ...
        if self._is_join:
            ...
            _params["limit"] = min(
                self._page_size, abs(self._limit - self._page_offset)
            )
        ...
        # If the request results in a QuartzBioError (ie bad filter) set the error.
        try:
        ...
        logger.debug(
            "query response took: %(took)d ms, total: %(total)d" % self._response
        )
    ...
    def export(self, format="json", follow=True, limit=None, **kwargs):
        from quartzbio import DatasetExport
    ...
    def migrate(self, target, follow=True, **kwargs):
        ...
        * commit_mode
        ...
        """
        from quartzbio import DatasetMigration
    ...
    def annotate(self, fields, **kwargs):
        from quartzbio.annotate import Annotator
    ...
    def join(self, query_b, key, key_b=None, prefix="b_", always_prefix=False):
        ...
        if query_params.get("filters"):
            filters = '[{{"and": [{}, {}]}}]'.format(
                base_filter, query_params["filters"][0]
            )
        else:
        ...
        if key_b == key and not (
            len(query_b_fields) == 1 and query_b_fields[0].name == key_b
        ):
        ...
        for field in query_b_fields:
            ...
            if field.is_list:
                ...
                expression = (
                    "[item for sublist in " + expression + " for item in sublist]"
                )
...
class BatchQuery(object):
    ...
    # Allows pre-setting a QuartzBioClient
    _client = None
    ...
    def _build_query(self):
        ...
        for i in self._queries:
            _params = i._build_query(
                ...
                limit=min(i._page_size, i._limit),
            )
    ...
    def execute(self, offset=0, **query):
        ...
        try:
            ...
            if (
                ...
                and self._output_format in ("csv", "tsv")
        ...
        logger.debug(
            "query response took: {} ms, total: {}".format(
                self._response["took"], self._response["total"]
            )
        )
        return _params, self._response
