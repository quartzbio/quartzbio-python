...
class VaultTests(QuartzBioTestCase):
    def test_vaults(self):
        vaults = self.client.Vault.all()
        vault = vaults.data[0]
        self.assertTrue("id" in vault, "Should be able to get id in vault")

        vault2 = self.client.Vault.retrieve(vault.id)
        self.assertEqual(
            vault, vault2, "Retrieving vault id {0} found by all()".format(vault.id)
        )
    ...
    def test_vault_versioning(self):
        ...
        self.addCleanup(
            VaultTests.clean_up_after_vault_versioning, vault, initial_versioning_status
        )
