...
from quartzbio.test.client_mocks import fake_migration_create


class TestDatasetMigrations(QuartzBioTestCase):
    @mock.patch("quartzbio.resource.DatasetMigration.create")
    def test_migration_from_query(self, Create):
        ...
        query = source.query(limit=10, fields=["my_field"]).filter(my_field=999)
        migration = query.migrate(target=target, commit_mode="overwrite", follow=False)
        ...
        self.assertEqual(migration.source_params["filters"], [("my_field", 999)])
    ...
    def test_migration_from_query_target_id(self, Create):
        ...
        query = source.query(limit=10, fields=["my_field"]).filter(my_field=999)
        migration = query.migrate(
            target=target.id, commit_mode="overwrite", follow=False
        )
        self.assertEqual(migration.source_id, source.id)
        self.assertEqual(migration.target_id, target.id)
        self.assertEqual(migration.commit_mode, "overwrite")
    ...
    def test_migration_target_dataset_object(self, Create):
        ...
        self.assertEqual(migration.commit_mode, "overwrite")
        self.assertEqual(migration.source_params["fields"], source_params["fields"])
        self.assertEqual(migration.source_id, source.id)
        self.assertEqual(migration.target_id, target.id)
