...
import quartzbio
from quartzbio import Query, Filter, GenomicFilter
...
class FilterTest(unittest.TestCase):
    def test_filter_basic(self):
        ...
        self.assertEqual(
            repr(~~f1), "<Filter [('price', 'Free')]>", '"not" of empty filter'
        )

        a = quartzbio.query.Filter(chr1="3")
        b = quartzbio.query.Filter(chr2="4")
        self.assertEqual(
            repr(a | b),
        ...
        f |= a
        self.assertEqual(repr(f), "<Filter [('chr1', '3')]>", "'or' with empty filter")
        self.assertEqual(
            repr(a), "<Filter [('chr1', '3')]>", "prior 'or' doesn't mung filter"
        )

        filters3 = (
            Filter(omim_id=144650) | Filter(omim_id=144600) | Filter(omim_id=145300)
        )
        self.assertEqual(
            repr(filters3),
    ...
    def test_raw_filters(self):
        ...
        f = Filter(raw_filter)
        self.assertEqual(f.filters, [["field_a", "value_a"]])
        f = Query._process_filters([f])
        self.assertEqual(f, [["field_a", "value_a"]])
        ...
        f = Filter(raw_filter)
        self.assertEqual(f.filters, [{"or": [["field_x", "value_x"]]}])
        f = Query._process_filters([f])
        self.assertEqual(f, [{"or": [["field_x", "value_x"]]}])
    ...
    def test_combined_raw_filters(self):
        ...
        self.assertEqual(
            f.filters, [{"and": [("field_x", "value_x"), ["field_a", "value_a"]]}]
        )
        f = Query._process_filters([f])
        self.assertEqual(f, [{"and": [("field_x", "value_x"), ["field_a", "value_a"]]}])

        # Combined, separate raw filters
        raw_filter = '[["field_a", "value_a"]]'
        f = Filter(raw_filter) | Filter(raw_filter)
        self.assertEqual(
            f.filters, [{"or": [["field_a", "value_a"], ["field_a", "value_a"]]}]
        )
        f = Query._process_filters([f])
        self.assertEqual(f, [{"or": [["field_a", "value_a"], ["field_a", "value_a"]]}])

        # Combined list of raw filters
        raw_filter = '[["field_a", "value_a"]]'
        f = Filter(raw_filter, raw_filter)
        self.assertEqual(
            f.filters, [{"and": [["field_a", "value_a"], ["field_a", "value_a"]]}]
        )
        f = Query._process_filters([f])
        self.assertEqual(f, [{"and": [["field_a", "value_a"], ["field_a", "value_a"]]}])
