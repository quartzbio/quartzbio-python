...
class ClientRateLimit(QuartzBioTestCase):
    ...
    def test_rate_limit(self):
        with patch("quartzbio.client.Session.request") as mock_request:
            mock_request.side_effect = lambda *args, **kw: self.fake_response()
            start_time = time.time()
            dataset = quartzbio.Dataset.retrieve(1)
            elapsed_time = time.time() - start_time
            self.assertTrue(
                isinstance(dataset, quartzbio.Dataset),
                "Got a dataset back (eventually)",
            )
            self.assertTrue(
                elapsed_time > 1.0,
                "Should have delayed for over a second; " "(was %s)" % elapsed_time,
            )
