...
from quartzbio.query import Filter
...
class BaseQueryTest(QuartzBioTestCase):
    ...
    def setUp(self):
        ...
        self.dataset = self.client.Object.get_by_full_path(self.TEST_DATASET_FULL_PATH)
    ...
    def test_basic(self):
        results = self.dataset.query().filter(
            omim_id__in=["OMIM:123631", "OMIM:123670", "OMIM:123690", "OMIM:306250"]
        )
    ...
    def test_empty_with_limit(self):
        ...
        limit = 100
        # bogus filter
        results = self.dataset.query(limit=limit).filter(omim_id="OMIM:99999")
        self.assertEqual(len(results), 0)
        self.assertEqual(results[:]._buffer, [])
        self.assertRaises(IndexError, lambda: results[0])
    ...
    def test_paging(self):
        ...
        self.assertEqual(len(set(map(str, _results))), num_pages * page_size)
    ...
    def test_paging_with_limit(self):
        ...
        self.assertEqual(len(set(map(str, _results))), limit)
    ...
    def test_slice_ranges_with_small_limit(self):
        ...
        # query returns 6
        limit = 1
        results = self.dataset.query(limit=limit).filter(mamit_trnadb__range=(2, 12))[
            0:4
        ]
        self.assertEqual(len(results), limit)
    ...
    def test_paging_and_slice_equivalence(self):
        ...
        def _query():
            return self.dataset.query(limit=10).filter(mamit_trnadb__range=(2, 12))
    ...
    def test_field_filters(self):
        ...
        results = self.dataset.query(limit=limit, exclude_fields=["entrez_id"])
    ...
    def test_target_fields(self):
        ...
        self.assertEqual(query.count(), 1)
        for record in query:
            assert "@quartzbio.com" in record["user"]
    ...
    def test_join_custom_prefix(self):
        query_a = self.dataset2.query(fields=["gene", "variant"], limit=1).filter(
            gene="MAN2B1"
        )
        query_b = self.dataset2.query(fields=["gene", "variant"])
    ...
    def test_join_with_list_key(self):
        ...
        annotator_params = {
            "pre_annotation_expression": "explode(record, ['clinical_significance'])"
        }
        query_a = (
            self.dataset2.query(
        ...
        query_b = (
            self.dataset2.query(fields=["clinical_significance", "gene"])
            .filter(gene="MAN2B1")
            .limit(10)
        )
    ...
    def test_join_with_list_values(self):
        ...
        query_b = (
            self.dataset2.query(fields=["clinical_significance", "gene", "variant"])
            .filter(gene="MAN2B1")
            .limit(10)
        )
    ...
    def test_join_pagination(self):
        # 50 records total
        query_a = self.dataset2.query(fields=["gene"], limit=50, page_size=10).filter(
            gene="MAN2B1"
        )
    ...
    def test_join_query_with_key_b(self):
        query_a = self.dataset.query(
            fields=["agr", "gene_symbol"], limit=50, page_size=10
        )
        query_b = self.dataset2.query(fields=["gene", "variant"])
    ...
    @unittest.skip(
        "Skip because API Host on GH pipelines doesn't host file at TEST_LARGE_TSV_FULL_PATH"
    )
    def test_query_large_file_into_dataframe(self):
        ...
        try:
            large_object = self.client.Object.get_by_full_path(
                self.TEST_LARGE_TSV_FULL_PATH
            )
