...
from quartzbio.test.client_mocks import fake_object_create, fake_object_save
from quartzbio.test.client_mocks import fake_dataset_create
...
class ObjectTests(QuartzBioTestCase):
    def test_object_paths(self):
        ...
        for vault in vaults:
            ...
            for file_ in list(vault.ls().quartzbio_objects())[:5]:
    ...
    def test_object_path_cases(self):
        ...
        test_cases = [
            ...
            [
                "myVault:/uploads_folder",
                "{0}:myVault:/uploads_folder".format(domain),
            ],  # noqa
    ...
    @mock.patch("quartzbio.resource.Dataset.create")
    @mock.patch("quartzbio.resource.Object.create")
    def test_object_dataset_getattr(self, ObjectCreate, DatasetCreate):
        ...
        ds_obj = self.client.Object.create(name="foo_dataset", object_type="dataset")
        file_ = self.client.Object.create(name="foo_file", object_type="file")
        folder_ = self.client.Object.create(name="foo_folder", object_type="folder")
    ...
    @mock.patch("quartzbio.resource.Object.create")
    @mock.patch("quartzbio.resource.Object.save")
    def test_object_remove_tag(self, ObjectCreate, ObjectSave):
        ObjectCreate.side_effect = fake_object_create
        ObjectSave.side_effect = fake_object_save

        tags = ["tag1", "tag2", "tag3"]
        file_ = self.client.Object.create(
            filename="foo_file", object_type="file", tags=tags
        )
        for tag in tags:
    ...
    def test_object_add_tag_non_iterable_or_string(self, ObjectCreate, ObjectSave):
        ...
        tags = "tag1"
        file_ = self.client.Object.create(filename="foo_file", object_type="file")
        file_.tag(tags)
    ...
    def test_object_set_metadata(self):
        folder = self.client.Object.get_or_create_by_full_path(
            "~/{}".format(uuid.uuid4()), object_type="folder"
        )
        ...
        # Test that direct assignement overrides QuartzBio object type with the given dictionary
        self.assertTrue(
            len(metadata) == 1
            and metadata.pop("foo_2") == "bar_2"
            and len(metadata) == 0
        )
    ...
    def test_object_set_metadata_empty_dict_list(self):
        ...
        # Test that 'metadata' is an empty dict (can't be a list)
        self.assertTrue(folder.metadata == {})
    ...
    def test_object_query(self, QuartzBioClientPost, ObjectCreate):
        ...
        file = self.client.Object.create(
            filename="foo_file.json", object_type="file", size=100
        )
        from .. import QuartzBioClient

        file._client = QuartzBioClient()

        QuartzBioClientPost.return_value = valid_response
        resp = file.query()
    ...
    def test_object_versioning(self):
        ...
        file = self.client.Object.create(
            filename="quartzbio-py-test.txt", object_type="file", vault_id=vault.id
        )

        self.addCleanup(
            ObjectTests.clean_up_after_object_versioning,
            vault,
            file,
            initial_versioning_status,
        )
    ...
    def test_shortcuts(self):
        ...
        folder = self.client.Object.create(
            filename=folder_name, object_type="folder", vault_id=vault.id
        )

        file = self.client.Object.create(
            filename="shorcut-test.txt", object_type="file", vault_id=vault.id
        )

        shortcut_folder_full_path = (
            vault.full_path + ":/{}-test-shortcut-folder".format(get_uuid_str())
        )
        ...
        shortcut_file_full_path = vault.full_path + ":/{}-test-shortcut-file".format(
            get_uuid_str()
        )
        ...
        shortcut_url = self.client.Object.create(
            filename=shortcut_url_name,
...
class ObjectUploadTests(QuartzBioTestCase):
    def setUp(self):
        super(ObjectUploadTests, self).setUp()
        self.tempdir = "/tmp/quartzbio-tests"
    ...
    def test_archive_non_files(self, DatasetCreate, ObjectCreate):
        ...
        folder_object = self.client.Object.create(
            name="foo_folder", object_type="folder"
        )
        with self.assertRaises(NotImplementedError):
            folder_object._archive("archive_folder")

        dataset_object = self.client.Object.create(
            name="foo_dataset", object_type="dataset"
        )
        with self.assertRaises(NotImplementedError):
            dataset_object._archive("archive_folder")

    def test_create_folder(self):
        folder_full_path = self.vault.full_path + ":/{}-test-folder".format(
            get_uuid_str()
        )
    ...
    def test_upload_file(self):
        ...
        expected_remote_path = (
            self.vault.full_path + ":/" + os.path.basename(local_path)
        )
    ...
    def test_skip_md5_match(self):
        ...
        with mock.patch("quartzbio.resource.Object.create") as ObjectCreate:
            self.client.Object.upload_file(
                local_path, remote_path, self.vault.full_path
            )
            self.assertEqual(ObjectCreate.call_count, 0)

    @mock.patch("quartzbio.resource.object.Object._get_timestamp")
    def test_archive_if_updated(self, TimeStamp):
        ...
        self.client.Object.upload_file(
            local_path, remote_path, self.vault.full_path, archive_folder="archive"
        )

        expected_archive_dir = (
            self.vault.full_path
            + ":/"
            + os.path.join("archive", remote_path.lstrip("/"))
        )
        dir_object = self.client.Object.get_by_full_path(expected_archive_dir)
        self.assertEqual(dir_object.object_type, "folder")

        expected_archive_full_path = (
            self.vault.full_path
            + ":/archive/{}_timestamp_string.txt.gz".format(filename_base)
        )
