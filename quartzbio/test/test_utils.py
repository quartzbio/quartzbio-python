...
from quartzbio.utils.files import check_gzip_path, separate_filename_extension

FILENAME_PARAMS = [
    {"filename": "test.txt", "base": "test", "ext": ".txt", "compression": ""},
    {
    ...
    {"filename": "test.txt.gz", "base": "test", "ext": ".txt", "compression": ".gz"},
]


class FilenameTests(QuartzBioTestCase, TestCase):
...
class GzipTest(QuartzBioTestCase):
    def test_gzip_file(self):
        path = os.path.join(os.path.dirname(__file__), "data")
        for yes_gzip in ["some_export.json.gz", "sample.vcf.gz"]:
